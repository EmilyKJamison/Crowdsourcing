...
import nominal.Worker;

public class EstimatorNom
{
    // If wanted, add annotators and/or items, but not categories
    // (Use Annotator1-6 for OI-sequences.)
    // NP Chunker: Louis XIV , King of France , built the Versailles Palace .
    String[] annotator1 = {"b", "i", "o", "b", "i", "i", "o", "o", "b", "i", "i", "o"}; // item1, item2, ...
    String[] annotator2 = {"b", "i", "i", "b", "i", "i", "i", "o", "b", "i", "i", "i"};
    String[] annotator3 = {"b", "b", "o", "b", "o", "b", "o", "o", "o", "b", "b", "o"};
    String[] annotator4 = {"b", "o", "o", "b", "o", "o", "o", "o", "o", "o", "b", "o"};
    String[] annotator5 = {"i", "o", "b", "i", "o", "o", "b", "b", "i", "o", "o", "b"};
    String[] annotator6 = {"o", "b", "i", "o", "b", "b", "i", "i", "o", "b", "b", "i"};
//    String[] annotator7 = {"b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b"};
//    String[] annotator8 = {"o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o"};
//    String[] annotator9 = {"b", "i", "i", "i", "i", "i", "o", "o", "b", "i", "i", "i"};
//    String[] annotator10= {"b", "i", "o", "b", "i", "i", "o", "o", "o", "b", "i", "o"};
    
    String[][] originalDataset = {annotator1, annotator2, 
            annotator3, 
            annotator4, 
            annotator5, 
            annotator6, 
...
//            annotator10
    ...
    public static final boolean FIXOIRULEINESTEP = false; //fix as part of E-step, iteratively
    public static final boolean FIXOIRULEATEND = true; // fix after convergence, non-iteratively
...
//                    predictedGolds[i][c] = (sumArray[c] / sumReliabilities[c]); 
    ...
    {
        for(int i=0;i<predictedGolds.length;i++){
                // if item is 'i' and previous item was 'o'
            if(predictedGolds[i][1] > predictedGolds[i][0] && predictedGolds[i][1] > predictedGolds[i][2]
                    && predictedGolds[i-1][2] > predictedGolds[i-1][0] && predictedGolds[i-1][2] > predictedGolds[i][1]){
                Double[] oldValues = {predictedGolds[i][0], predictedGolds[i][1], predictedGolds[i][2]};
                while(predictedGolds[i][1] > predictedGolds[i][0]){
                    // increase 'b' +1, and decrease 'i' and 'o' proportionally (by i/i+o or o/i+o)
                    double nonbtotal = predictedGolds[i][1] + predictedGolds[i][2];
                    predictedGolds[i][0]++;
                    predictedGolds[i][1] = predictedGolds[i][1] - (predictedGolds[i][1] / nonbtotal);
                    predictedGolds[i][2] = predictedGolds[i][2] - (predictedGolds[i][2] / nonbtotal);
                }
                if(VERBOSE){
                    System.out.print("ln139 OI-Corrected from ");
                    System.out.print(oldValues[0] + " " + oldValues[1] + " " + oldValues[2] + ", to ");
                    System.out.print(predictedGolds[i][0] + " " + predictedGolds[i][1] + " " + predictedGolds[i][2]);
                }
            }
        }
    }
    ...
    protected void printFinal(){
        System.out.println("\n Final Results:");
        if(VERBOSE){
            System.out.println("High values mean more bias.");
            for(Worker worker: workers){
                System.out.print("Worker " + worker.getId() + " bias: ");
            ...
            System.out.println("\nLow values mean more reliability.");
            for(Worker worker: workers){
                System.out.print("Worker " + worker.getId() + " reliability: ");
